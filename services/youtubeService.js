...
async function listRecentVideos(limit = 10) {
  ...
  return (playlistResponse.data.items || [])
    ...
    .map((item) => ({
      ...
      embedUrl: `https://www.youtube.com/embed/${item.contentDetails.videoId}?controls=0&disablekb=1&fs=0&iv_load_policy=3&rel=0&playsinline=1&enablejsapi=1&origin=https://dr.africacold.fr`,
    }));
}
...
async function uploadVideo({ stream, mimeType = "video/webm", title, description = "" }) {
  ...
  return {
    id,
    embedUrl: `https://www.youtube.com/embed/${id}?controls=0&disablekb=1&fs=0&iv_load_policy=3&rel=0&playsinline=1&enablejsapi=1&origin=https://dr.africacold.fr`,
    privacyStatus: "unlisted",
  };
}
